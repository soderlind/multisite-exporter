...
/**
 * Implements Multisite Exporter WP-CLI commands.
 *
 * @when before_wp_load
 */
class Multisite_Exporter_CLI_Command {

	/**
	 * Shows information about Multisite Exporter commands.
	 * 
	 * ## EXAMPLES
	 *
	 *     # Display basic help information
	 *     $ wp multisite-exporter
	 *
	 * @when before_wp_load
	 */
...
WP_CLI::add_command( 'multisite-exporter', 'Multisite_Exporter_CLI_Command' );
...
if ( ! class_exists( 'ME_CLI_Command' ) ) {
	...
	if ( ! defined( 'MULTISITE_EXPORTER_VERSION' ) ) {
		define( 'MULTISITE_EXPORTER_VERSION', '1.2.4' );
	}
	 ...
	 */
	function multisite_exporter_get_sites_to_export( $site_ids ) {
	 ...
	 */
	function multisite_exporter_create_zip_archive( $files, $destination ) {
	 ...
	 */
	function multisite_exporter_load_export_functionality() {
		// Make sure WordPress export functions are available
	...
	/**
	 * Export subcommand for Multisite Exporter.
	 *
	 * ## OPTIONS
	 *
	 * [--site_ids=<ids>]
	 ...
	 */
	$multisite_exporter_export = function ($args, $assoc_args) {
		...
		// Get sites to export
		$sites = multisite_exporter_get_sites_to_export( $site_ids );
		if ( empty( $sites ) ) {
			return;
		}
		...
		// Ensure export functionality is loaded
		multisite_exporter_load_export_functionality();
		...
		if ( count( $export_files ) > 1 ) {
			...
			multisite_exporter_create_zip_archive( $export_files, $zip_file );
	...
	WP_CLI::add_command( 'multisite-exporter export', $multisite_exporter_export, array(
		'shortdesc' => 'Exports content from one or more sites in a multisite network.',
		'synopsis'  => array(
			array(
				'type'        => 'assoc',
				'name'        => 'site_ids',
				'description' => 'Comma-separated list of site IDs to export. Default is all sites.',
				'optional'    => true,
			),
			array(
				'type'        => 'assoc',
				'name'        => 'content',
				'description' => 'Comma-separated list of content types to export: all, posts, pages, media. Default is all.',
				'optional'    => true,
			),
			array(
				'type'        => 'assoc',
				'name'        => 'start_date',
				'description' => 'Optional start date for filtering content in YYYY-MM-DD format.',
				'optional'    => true,
			),
			array(
				'type'        => 'assoc',
				'name'        => 'end_date',
				'description' => 'Optional end date for filtering content in YYYY-MM-DD format.',
				'optional'    => true,
			),
		),
	) );
